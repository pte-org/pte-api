...
/** Records the "still alive" presence signal for the parallel connectivity-monitoring feature — entirely separate storage from the attempt row itself, by design. */
@Service
public class HeartbeatService {
     ...
     *
     * <p>{@code REQUIRES_NEW}: two racing first-ever calls for the same attempt
     * could both miss the {@code findByAttemptId} and both attempt an insert —
     * Postgres poisons the WHOLE transaction once the loser's insert hits the
     * unique constraint on {@code attempt_id}, so catching the exception alone
     * (in the same transaction as the caller) does NOT stop it from resurfacing
     * as a {@code TransactionSystemException} at the caller's own commit.
     * Running this in its own physical transaction contains that abort to just
     * this call — a best-effort, fire-and-forget presence write that should
     * never be able to fail the caller's own unrelated work (or vice versa:
     * this write always durably lands independent of whatever the caller does
     * afterward).
     */
