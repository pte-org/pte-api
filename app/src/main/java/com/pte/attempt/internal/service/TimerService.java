...
/**
 * The client owns the whole-attempt countdown and auto-submit UX — there is
 * no server-side deadline enforcement (prep/response windows, a `/timer`
 * poll endpoint). What remains here has no other home: resetting per-task
 * replay counters when a new task starts, and tracking the section-scoped
 * shared-budget window (currently only READING) so
 * {@link #resolveEffectiveResponseSeconds} can hand the client a correct
 * live value on every per-task fetch — no absolute deadline, no polling
 * required.
 *
...
public class TimerService {
     ...
     * {@code item}. For a task-scoped item this is just the item's own static
     * values, unchanged. For a section-scoped item (READING), the item's own
     * {@code prepSeconds}/{@code responseSeconds} are just that item's
     * individual slice — the value the client needs is the *live remaining
     * shared budget*, computed fresh here from {@link ExamAttempt#getSectionStartedAt()}.
     * Must be called AFTER {@link #startTask}, which is what sets/maintains
     * that attempt field.
     */
