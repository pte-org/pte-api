 ...
 * exactly the "slow/unreliable external call" this split exists for. {@code
 * EMAIL_QUEUE} dead-letters to {@code EMAIL_DLQ} after 3 failed delivery
 * attempts — the recoverer throws {@link AmqpRejectAndDontRequeueException}
 * once retries are exhausted, which the container turns into a
 * NACK-without-requeue, routed to the DLX by the queue's {@code
 * x-dead-letter-exchange}. A stuck job lands somewhere host-visible ({@code
