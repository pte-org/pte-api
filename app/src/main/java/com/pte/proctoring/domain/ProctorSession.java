package com.pte.proctoring.domain;
...
/**
 * A proctor's live monitoring window over one {@code ExamSession} (session
 * module's aggregate, referenced by {@code publicId} only). {@code
 * lastSequenceNo}/{@code lastHash} are the running head of this session's
 * tamper-evident {@link ViolationEvent} hash chain; both advance in the same
 * transaction as each new violation row, so there is no race window between
 * "read the current head" and "write the next link."
 */
@Entity
