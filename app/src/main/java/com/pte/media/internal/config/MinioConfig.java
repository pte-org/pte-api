package com.pte.media.internal.config;
...
public class MinioConfig {
     ...
     * rather than left to the SDK's auto-detection: without a client-provided
     * region, the MinIO Java SDK resolves it via a real {@code GetBucketLocation}
     * network call on first use per bucket — which this client, configured with
     * the (deliberately host-external) public endpoint, cannot make from inside
     * this container. Pinning the region avoids that call entirely, keeping this
     * client's {@code getPresignedObjectUrl} the pure local HMAC computation
     * it's meant to be. The admin-scoped {@link #minioClient} bean above doesn't
