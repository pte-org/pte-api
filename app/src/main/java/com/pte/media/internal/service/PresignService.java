...
import com.pte.media.internal.exception.InvalidWavFileException;
...
import com.pte.media.internal.exception.PresignFailedException;
 ...
 * module's own request body (avoids proxying large binaries through the API
 * tier).
 */
@Service
public class PresignService {
    ...
    private static final Set<String> ALLOWED_CONTENT_TYPES = Set.of(
            ...
            // Speaking's DESCRIBE_IMAGE prompt images.
    ...
    // this module's own server-to-server MinIO calls elsewhere).
    ...
    /**
     * Trusted application-call surface only (see {@code com.pte.media.MediaService}).
     * Tenant-scoped at the database layer (not just trusted from the caller) —
     * the calling module's own entitlement check is a separate, upper-layer
     * guard; this lookup must not rely on it alone (defense in depth).
     */
     ...
     * Package-private (not {@code private}) so tests can stub it directly via
     * {@code Mockito.spy} without needing to fabricate a real MinIO SDK
     * response type.
     */
