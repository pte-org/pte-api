...
/**
 * Two chains: {@code /internal/**} (service-to-service, API-key — reporting
 * gains its first internal endpoint in rabbitmq-outbox-migration Phase 9, the
 * new-instance full-bootstrap rebuild trigger) evaluated first, then the
 * normal JWT resource-server chain (student/host report access, and now the
 * single-tenant rebuild trigger). See authoring's {@code SecurityConfig} for
 * the identical base pattern; the bootstrap filter is new here (Phase 9).
 */
@Configuration
