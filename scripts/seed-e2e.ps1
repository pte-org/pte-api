...
.SYNOPSIS
  ...
  Seeds, via the Nginx edge's real public REST API only (never in-process, never
...
.DESCRIPTION
  Talks ONLY to the Nginx edge (http://localhost:8080 by default) â€” the exact
  ...
  PREREQUISITE (one-time, cannot be done via API â€” pte-api has no
  ...
  1. Generate a BCrypt hash for your chosen bootstrap password. `htpasswd`
     ...
     least one dev machine this was verified against) â€” the reliable,
     ...
     'Password123!' | cut -d: -f2` works too â€” both produce the same
     BCrypt format.) Copy the printed hash (starts with $2a$/$2b$/$2y$,
     strip the leading ":" if using the Docker form) â€” the SQL below uses
  ...
  2. Run this SQL once, e.g. `psql -h localhost -U iam_svc -d iam`:
       ...
       RETURNING id;
       -- capture the printed id (an integer, e.g. 1) â€” use it as <USER_ID> below.
       ...
       -- BaseEntity.java â€” this is the one deliberate exception).
  ...
  object â€” SnapshotPublishService.publish() has no server-side idempotency
  ...
  the state file too), this script cannot reconcile the two â€” delete both
  ...
  sentence â€” genuinely audible (unlike pte-app's own silent
...
.NOTES
  ...
  else â€” there is no Spring @Profile gate for an external script, so this
...
param(
    ...
    # (not merged into) Read Aloud's above â€” keeps the already-verified Read
    ...
    # â€” a short, genuinely audible synthesized tone, not one of the silent
    ...
    # First Listening e2e verification (MC_LISTENING_SINGLE) â€” the other 7
    ...
    # â€” see scripts/fixtures/README or the generation one-liner in this
...
if ($ApiBaseUrl -notmatch 'localhost|127\.0\.0\.1') {
    throw "Refusing to run against a non-local Nginx edge URL: $ApiBaseUrl (this script is dev/local-only)"
}
...
# ---------------------------------------------------------------------------
# State file â€” the ONLY recovery path for the published snapshot's publicId
...
# state file (from before that property existed â€” e.g. one saved by
...
# silently creating it, unlike a Hashtable â€” so every property this script
    ...
    try {
        ...
        if ($resp) {
            ...
            try {
                ...
                # Response stream not readable â€” fall through with $errorBody still $null.
            }
            ...
            # No HTTP response at all â€” connection-level failure (Nginx edge not
...
Write-Step "Target Nginx edge: $ApiBaseUrl"
...
$tenantsResp = Invoke-Api -Method Get -Path '/api/v1/tenants' -Token $platformToken
...
if ($null -eq $tenant) {
    ...
    # match ^[a-z0-9-]{3,32}$ â€” it becomes the prefix of every student
    ...
    $createTenantResp = Invoke-Api -Method Post -Path '/api/v1/tenants' -Token $platformToken -Body @{
        code             = $tenantCode
        name             = $TenantName
...
#    PLATFORM_ADMIN caller has for an arbitrary tenant's users â€” GET /users
#    itself is caller-tenant-scoped and returns nothing for a platform
#    caller).
# ---------------------------------------------------------------------------
Write-Step "Resolving users for tenant $($tenant.publicId)..."
$tenantUsersResp = Invoke-Api -Method Get -Path "/api/v1/users/by-tenant/$($tenant.publicId)" -Token $platformToken
...
if ($null -eq $hostUser) {
    ...
    $createHostResp = Invoke-Api -Method Post -Path '/api/v1/users' -Token $platformToken -Body @{
        email    = $HostEmail
...
if ($null -eq $studentUser) {
    ...
    $createStudentResp = Invoke-Api -Method Post -Path '/api/v1/users' -Token $platformToken -Body @{
        email    = $StudentEmail
...
$hostLoginResp = Invoke-Api -Method Post -Path '/api/v1/auth/login' -Body @{
    username = $HostEmail
    password = $HostPassword
}
...
# ---------------------------------------------------------------------------
# 5. READ_ALOUD question (visibility PRIVATE â€” SHARED/platform-bank
...
$questionsResp = Invoke-Api -Method Get -Path '/api/v1/questions' -Token $hostToken
$question = Find-First -Items $questionsResp.data -Property 'title' -Value $QuestionTitle
if ($null -eq $question) {
    Write-Step "Question not found - creating..."
    $createQuestionResp = Invoke-Api -Method Post -Path '/api/v1/questions' -Token $hostToken -Body @{
...
$blueprintsResp = Invoke-Api -Method Get -Path '/api/v1/blueprints' -Token $hostToken
...
if ($null -eq $blueprint) {
    ...
    $createBlueprintResp = Invoke-Api -Method Post -Path '/api/v1/blueprints' -Token $hostToken -Body @{
        name  = $BlueprintName
        items = @(
...
# ---------------------------------------------------------------------------
# 7. Published snapshot â€” see header comment: NOT re-derivable via any API
...
if ($blueprint.status -eq 'PUBLISHED') {
    ...
    $publishResp = Invoke-Api -Method Post -Path "/api/v1/blueprints/$($blueprint.publicId)/publish" -Token $hostToken
...
# ---------------------------------------------------------------------------
# 8. Scheduling session â€” opensAt/closesAt MUST be computed relative to now
...
$sessionsResp = Invoke-Api -Method Get -Path '/api/v1/sessions' -Token $hostToken
...
if ($null -eq $session) {
    ...
    $createSessionResp = Invoke-Api -Method Post -Path '/api/v1/sessions' -Token $hostToken -Body @{
        name             = $SessionName
...
# ---------------------------------------------------------------------------
# 9. Composition â€” setComposition() clears-then-reapplies server-side, so
...
if ($hasReadAloudComposition) {
    ...
    Invoke-Api -Method Put -Path "/api/v1/sessions/$($session.publicId)/composition" -Token $hostToken -Body @{
        items = @(
            # No override â€” real production default (40s). Historical note: this
            ...
            # Server-side deadline enforcement â€” TimerState included â€” was deleted
...
# ---------------------------------------------------------------------------
# 10. Open â€” open() is a trivial idempotent status setter server-side, but
...
if ($session.status -eq 'OPEN') {
    ...
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($session.publicId)/open" -Token $hostToken | Out-Null
...
$enrollmentsResp = Invoke-Api -Method Get -Path "/api/v1/sessions/$($session.publicId)/enrollments" -Token $hostToken
...
if ($null -eq $enrollment) {
    ...
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($session.publicId)/enrollments" -Token $hostToken -Body @{
...
# ---------------------------------------------------------------------------
# 12. Repeat Sentence audio media â€” uploaded via the same 3-step presigned
...
#     NOT re-derivable via any API call once uploaded â€” the state file is
...
if ($State.repeatSentenceAudioMediaPublicId) {
    ...
    $requestUploadResp = Invoke-Api -Method Post -Path '/api/v1/objects' -Token $hostToken -Body @{
        contentType = 'audio/wav'
        # Opts into WAV-only validation + WAV-header duration extraction at
        # complete-upload time (plans/phat-speaking-dynamic-prep-timing) â€”
    ...
    Invoke-Api -Method Post -Path "/api/v1/objects/$mediaPublicId/complete" -Token $hostToken | Out-Null
...
# ---------------------------------------------------------------------------
# 13. Repeat Sentence question â€” audioPromptRef is the only required field
...
$rsQuestionsResp = Invoke-Api -Method Get -Path '/api/v1/questions' -Token $hostToken
...
if ($null -eq $repeatSentenceQuestion) {
    Write-Step "Question not found - creating..."
    $createRsQuestionResp = Invoke-Api -Method Post -Path '/api/v1/questions' -Token $hostToken -Body @{
...
$rsBlueprintsResp = Invoke-Api -Method Get -Path '/api/v1/blueprints' -Token $hostToken
...
if ($null -eq $repeatSentenceBlueprint) {
    Write-Step "Blueprint not found - creating..."
    $createRsBlueprintResp = Invoke-Api -Method Post -Path '/api/v1/blueprints' -Token $hostToken -Body @{
...
# ---------------------------------------------------------------------------
# 15. Published snapshot â€” same not-re-derivable-via-API limitation as step 7.
# ---------------------------------------------------------------------------
...
if ($repeatSentenceBlueprint.status -eq 'PUBLISHED') {
    ...
    Write-Step "Blueprint not yet published - publishing..."
    $rsPublishResp = Invoke-Api -Method Post -Path "/api/v1/blueprints/$($repeatSentenceBlueprint.publicId)/publish" -Token $hostToken
...
$rsSessionsResp = Invoke-Api -Method Get -Path '/api/v1/sessions' -Token $hostToken
...
if ($null -eq $repeatSentenceSession) {
    ...
    $createRsSessionResp = Invoke-Api -Method Post -Path '/api/v1/sessions' -Token $hostToken -Body @{
        ...
        # PRACTICE for the same reason as the Read Aloud session (step 8) â€”
...
if ($hasRepeatSentenceComposition) {
    ...
    Invoke-Api -Method Put -Path "/api/v1/sessions/$($repeatSentenceSession.publicId)/composition" -Token $hostToken -Body @{
        items = @(
            # No override â€” real production default (prepSeconds 10 /
...
if ($repeatSentenceSession.status -eq 'OPEN') {
    ...
    Write-Step "Opening session..."
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($repeatSentenceSession.publicId)/open" -Token $hostToken | Out-Null
    Write-Ok "Session opened."
}
...
# ---------------------------------------------------------------------------
Write-Step "Resolving enrollment for student $($studentUser.publicId)..."
$rsEnrollmentsResp = Invoke-Api -Method Get -Path "/api/v1/sessions/$($repeatSentenceSession.publicId)/enrollments" -Token $hostToken
...
if ($null -eq $rsEnrollment) {
    Write-Step "Student not enrolled - enrolling..."
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($repeatSentenceSession.publicId)/enrollments" -Token $hostToken -Body @{
        studentPublicId = $studentUser.publicId
    } | Out-Null
    Write-Ok "Enrolled student."
} else {
...
# ---------------------------------------------------------------------------
# 20. MC Listening Single audio media â€” same 3-step presigned flow as the
...
if ($State.listeningAudioMediaPublicId) {
    ...
    $listeningUploadResp = Invoke-Api -Method Post -Path '/api/v1/objects' -Token $hostToken -Body @{
        contentType = 'audio/wav'
        audioPrompt = $true
    }
    ...
    Invoke-Api -Method Post -Path "/api/v1/objects/$listeningMediaPublicId/complete" -Token $hostToken | Out-Null
...
# ---------------------------------------------------------------------------
# 21. MC_LISTENING_SINGLE question â€” audioPromptRef + 4 options, exactly one
...
#     promptText: PteTaskType.MC_LISTENING_SINGLE doesn't require it â€” the
...
$listeningQuestionsResp = Invoke-Api -Method Get -Path '/api/v1/questions' -Token $hostToken
...
if ($null -eq $listeningQuestion) {
    Write-Step "Question not found - creating..."
    $createListeningQuestionResp = Invoke-Api -Method Post -Path '/api/v1/questions' -Token $hostToken -Body @{
...
$listeningBlueprintsResp = Invoke-Api -Method Get -Path '/api/v1/blueprints' -Token $hostToken
...
if ($null -eq $listeningBlueprint) {
    Write-Step "Blueprint not found - creating..."
    $createListeningBlueprintResp = Invoke-Api -Method Post -Path '/api/v1/blueprints' -Token $hostToken -Body @{
...
# ---------------------------------------------------------------------------
# 23. Published snapshot â€” same not-re-derivable-via-API limitation as step 7.
# ---------------------------------------------------------------------------
...
if ($listeningBlueprint.status -eq 'PUBLISHED') {
    ...
    Write-Step "Blueprint not yet published - publishing..."
    $listeningPublishResp = Invoke-Api -Method Post -Path "/api/v1/blueprints/$($listeningBlueprint.publicId)/publish" -Token $hostToken
...
$listeningSessionsResp = Invoke-Api -Method Get -Path '/api/v1/sessions' -Token $hostToken
...
if ($null -eq $listeningSession) {
    ...
    $createListeningSessionResp = Invoke-Api -Method Post -Path '/api/v1/sessions' -Token $hostToken -Body @{
...
if ($hasListeningComposition) {
    ...
    Invoke-Api -Method Put -Path "/api/v1/sessions/$($listeningSession.publicId)/composition" -Token $hostToken -Body @{
        items = @(
...
if ($listeningSession.status -eq 'OPEN') {
    ...
    Write-Step "Opening session..."
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($listeningSession.publicId)/open" -Token $hostToken | Out-Null
    Write-Ok "Session opened."
}
...
# ---------------------------------------------------------------------------
Write-Step "Resolving enrollment for student $($studentUser.publicId)..."
$listeningEnrollmentsResp = Invoke-Api -Method Get -Path "/api/v1/sessions/$($listeningSession.publicId)/enrollments" -Token $hostToken
...
if ($null -eq $listeningEnrollment) {
    Write-Step "Student not enrolled - enrolling..."
    Invoke-Api -Method Post -Path "/api/v1/sessions/$($listeningSession.publicId)/enrollments" -Token $hostToken -Body @{
        studentPublicId = $studentUser.publicId
    } | Out-Null
    Write-Ok "Enrolled student."
} else {
